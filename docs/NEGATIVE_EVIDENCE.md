...
### NE-003 speculative-greedy-with-transition-sketch-drafter

`claim_id: local-frankenmtp-greedy-drafter-v1`; `evidence_id: measure_frankenmtp_drafter_greedy_acceptance_across_utterance + speculative_greedy_mode_produces_identical_code_streams_to_sequential tests in crates/ftts-model-qwen (480 token proposals across 32 frames)`; `status: NEGATIVE`; `model_source_commit: HEAD`; `fixture_sha256: not-applicable (synthetic and canonical greedy execution)`; `cpu_features: all (algorithmic acceptance property)`; `command_env: FTTS_SPEC_MTP=1 cargo test -p ftts-model-qwen measure_frankenmtp_drafter_greedy_acceptance_across_utterance`; `kill_switch: FTTS_SPEC_MTP=1 (defaults to 0 / unarmed; unarmed path is structurally unchanged)`; `before_after: FrankenMtpDrafter (previous-frame residuals + 64-bucket transition sketch) achieves p_token = 0.0000 and alpha_full = 0.0000 under strict-greedy argmax verification, well below break-even thresholds (p* ≈ 94-95%, alpha_full* ≈ 40-46% per docs/truth-pack/ACCEPTANCE_SURFACE.json). Speculative cost T_draft + T_verify + T_repair exceeds sequential baseline T_seq on every frame`; `equivalence: exact token equality verified in speculative_greedy_mode_produces_identical_code_streams_to_sequential (speculation-on equals sequential byte-for-byte)`; `killing_metric: argmax acceptance probability vs alpha*(SKU)`; `disposition: REVERT (unarmed by default; kill-switch FTTS_SPEC_MTP retained for future learned drafters / Phase 5 drafter ladder)`; `do_not_retry: with zero-compute transition sketch or previous-frame copy; drafter ladder 2-5 requires a learned/distilled drafter head (p5-drafter-ladder-0wk)`; `tally_local_w_l_n: 0/3/0`.
