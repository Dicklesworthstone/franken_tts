...
3. Enroll it as your default voice:
   ...
   Enrollment computes a 1,024-float speaker embedding from the audio alone — **no transcript is needed**. Any container at any sample rate works — 44.1 and 48 kHz phone and Mac voice memos included. Compressed references are converted by the system decoder; `.wav` and `.flac` are read directly and resampled in-process by a windowed-sinc (Lanczos-6) kernel. Audio already at 24 kHz is passed through untouched. The enroll step warns about recordings that will clone poorly (background noise, clipping, whispering, multiple speakers), and `--denoise` applies opt-in noise removal: a pure-Rust port of the FastEnhancer-S neural denoiser (207 K parameters, fetched by `ftts pull`, proven to 125 dB parity against its PyTorch reference) when the artifact is present, falling back to classic spectral subtraction when it is not. On a static-hiss test reference the neural engine lowers the pause floor by 65 dB and lands the enrolled voice measurably closer to its clean-source enrollment; `FTTS_DENOISE_ENGINE=omlsa` forces the classic engine.
